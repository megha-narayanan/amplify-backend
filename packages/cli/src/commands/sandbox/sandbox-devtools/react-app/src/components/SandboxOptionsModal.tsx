  ...
  const handleConfirm = () => {
    ...
    if (streamFunctionLogs) {
      ...
      if (logsFilterInput.trim()) {
        options.logsFilter = logsFilterInput.split(',').map(item => item.trim());
      }
      ...
      footer={
        <Box float="right">
          <SpaceBetween direction="horizontal" size="xs">
            ...
            <Button variant="primary" onClick={handleConfirm}>Start Sandbox</Button>
          </SpaceBetween>
        </Box>
      }
    >
      <SpaceBetween size="l">
        <ColumnLayout columns={2}>
          <FormField label="AWS Profile" description="The AWS profile to use for the sandbox">
            <Input
              value={profile}
          ...
          <FormField label="Sandbox Identifier" description="A unique identifier for the sandbox (1-15 characters, alphanumeric and hyphens only)">
            <Input
              value={identifier}
        ...
        <FormField label="Directory to Watch" description="Directory to watch for file changes (defaults to ./amplify)">
          <Input
            value={dirToWatch}
        ...
        <FormField label="Exclude Paths" description="Comma-separated list of paths or glob patterns to ignore">
          <Input
            value={excludeInput}
        ...
        <ColumnLayout columns={2}>
          <FormField label="Outputs Format" description="amplify_outputs file format">
            <Select
              selectedOption={outputsFormat ? { label: outputsFormat, value: outputsFormat } : null}
              onChange={({ detail }) => setOutputsFormat(detail.selectedOption?.value || '')}
              options={[
                { label: "Default", value: "" },
          ...
          <FormField label="Outputs Version" description="Version of the configuration">
            <Select
              selectedOption={outputsVersion ? { label: outputsVersion, value: outputsVersion } : null}
              onChange={({ detail }) => setOutputsVersion(detail.selectedOption?.value || '')}
              options={[
                { label: "Default", value: "" },
                { label: "v0", value: "0" },
                { label: "v1", value: "1" }
        ...
        <FormField label="Outputs Directory" description="Directory where amplify_outputs is written">
          <Input
            value={outputsOutDir}
        ...
        {streamFunctionLogs && (
          <>
            <FormField label="Logs Filter" description="Comma-separated regex patterns to filter logs from matched functions">
              <Input
            ...
            <FormField label="Logs Output File" description="File to append the streaming logs">
              <Input
                value={logsOutFile}
