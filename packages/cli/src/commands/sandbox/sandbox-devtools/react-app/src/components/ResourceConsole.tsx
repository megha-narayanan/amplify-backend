...
const ResourceConsole: React.FC<ResourceConsoleProps> = ({
  ...
  useEffect(() => {
    ...
    const handleLogStreamStatus = (data: {
      ...
      console.log(
        `ResourceConsole: Log stream status for ${data.resourceId}: ${data.status}`,
      );
      ...
      if (data.status === 'active' || data.status === 'already-active') {
        ...
        if (
          selectedLogResource &&
          selectedLogResource.physicalResourceId === data.resourceId
        ) {
    ...
    const handleLogStreamError = (data: {
      ...
      console.error(
        `ResourceConsole: Log stream error for ${data.resourceId}: ${data.error}`,
      );
