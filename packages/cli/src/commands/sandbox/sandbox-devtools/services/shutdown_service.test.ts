...
void describe('ShutdownService', () => {
  ...
  beforeEach(() => {
    ...
    mockIo = { emit: mock.fn(), close: mock.fn() } as unknown as Server;
    mockServer = { close: mock.fn() } as unknown as HttpServer;
    mockStorageManager = { clearAll: mock.fn() } as unknown as LocalStorageManager;
  ...
  void describe('shutdown', () => {
    void it('stops running sandbox and clears storage', async () => {
      ...
      assert.strictEqual((mockSandbox.stop as unknown as MockFn).mock.callCount(), 1);
      assert.strictEqual((mockStorageManager.clearAll as unknown as MockFn).mock.callCount(), 1);
      assert.strictEqual((mockIo.emit as unknown as MockFn).mock.callCount(), 1);
      assert.strictEqual((mockIo.close as unknown as MockFn).mock.callCount(), 1);
      assert.strictEqual((mockServer.close as unknown as MockFn).mock.callCount(), 1);
    });

    void it('skips sandbox stop when not running', async () => {
      const stoppedService = new ShutdownService(
        mockIo, mockServer, mockStorageManager, mockSandbox, async () => 'stopped'
      );

      await stoppedService.shutdown('SIGTERM', false);

      assert.strictEqual((mockSandbox.stop as unknown as MockFn).mock.callCount(), 0);
      assert.strictEqual((mockStorageManager.clearAll as unknown as MockFn).mock.callCount(), 1);
    });
